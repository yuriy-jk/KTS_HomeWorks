...
from project_server.web.exceptions import Error
...
async def error_mw(request, handler):
    try:
        ...
        return web.json_response(
            {"code": Error.code, "description": Error.description}, status=Error.status
        )
...
async def auth_mw(request: web.Request, handler):
    ...
    if session_id:
        session = request.app["store"].session.get_by_id(session_id)
        if session:
